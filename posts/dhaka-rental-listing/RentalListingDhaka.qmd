---
title: "Exploring Dhaka's Rental Market: A Spatial Analysis of Apartment Prices"
...
Libraries That I will use:[@sf; @raster; @dplyr; @spData; @terra; @OpenStreetMap; @osmdata; @leaflet]
...
Loading the shape files of three different layers of administrative distribution. I downloaded this data from Humanitarian Data Exchange[@hdx-bangladesh]. There are 4 different administrative area categories in Bangladesh. ADMIN1: the divisions, ADMIN2: the districts, ADMIN3: upazillas/sectors (the latter for cities), ADMIN4: unions/wards (the former for rural areas, the latter for cities). Since I am only interested in the city of Dhaka which is a part of the district Dhaka, I am only interested in the layers ADMIN3 and ADMIN4. I will also call areas from these layers as ADMIN3 area and ADMIN4 area respectively.
...
Now let's read the rental price data. This is from a Kaggle Dataset[@kaggle-bangladesh-property]. The provider scraped it from the popular website [bproperty.com](bproperty.com) and released it under CC0 license.
